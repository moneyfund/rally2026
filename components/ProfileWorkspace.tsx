...
function profileFromData(uid: string, data: Record<string, unknown>, user: User): GerminaProfile {
  ...
  const publicCoords = readCoordinates(data.coordinates);
  ...
  return {
    ...
    coordinates: publicCoords,
    locationPublic: typeof data.locationPublic === "boolean" ? data.locationPublic : Boolean(publicCoords),
...
export function ProfileWorkspace() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        ...
        const privateLocationRef = doc(db, "users", user!.uid, "privateLocation", "current");
        const [snapshot, privateLocationSnapshot] = await Promise.all([
          getDoc(profileRef),
          getDoc(privateLocationRef).catch(() => null),
        ]);
        ...
        const privateCoordinates = privateLocationSnapshot?.exists()
          ? readCoordinates(privateLocationSnapshot.data().coordinates)
          : null;
        if (privateCoordinates) next.coordinates = privateCoordinates;
  ...
  const completion = useMemo(() => {
    const checks = [profile.name, profile.profession, profile.description, profile.location, profile.services.length, avatar];
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const selectedCoordinates = profile.coordinates;
      const publicCoordinates = profile.locationPublic ? selectedCoordinates : null;

      await setDoc(privateLocationRef, {
      ...
      setNotice(profile.locationPublic
        ? "Perfil guardado. La ubicación seleccionada está publicada."
        : "Perfil guardado. La ubicación quedó guardada de forma privada.");
    } catch (caught) {
  ...
  const heroStyle = profile.coverUrl
    ? { backgroundImage: `linear-gradient(90deg, rgba(5,34,61,.94) 0%, rgba(9,70,117,.78) 48%, rgba(21,111,169,.48) 100%), url(${profile.coverUrl})` }
    : undefined;

  return (
    <main className="profile-private-page">
      <section className="profile-dashboard-hero profile-cover-hero" style={heroStyle}>
        <div className="shell profile-dashboard-hero-inner">
          <div>
            ...
            <p>Administrá cómo te ven, qué ofrecés y dónde pueden encontrarte.</p>
          </div>
          <div className="profile-hero-tools">
            ...
            <button type="button" className="profile-cover-button" disabled={uploadingCover} onClick={() => coverInputRef.current?.click()}><ImagePlus size={16} /> {uploadingCover ? "Subiendo portada..." : "Cambiar portada"}</button>
      ...
      <div className="shell profile-dashboard-shell">
        <aside className="profile-sidebar">
          <div className="profile-avatar-card">
            ...
            <button type="button" className="profile-upload-button" disabled={uploadingAvatar} onClick={() => avatarInputRef.current?.click()}><Camera size={16} /> {uploadingAvatar ? "Subiendo..." : "Cambiar foto"}</button>
        ...
        <div className="profile-editor-column">
          ...
          <section id="informacion" className="profile-editor-card reveal-card">
            <div className="profile-card-heading"><div><span className="profile-section-number">01</span><div><h2>Información pública</h2><p>Tu presentación principal dentro de Germina.</p></div></div><span className="profile-privacy-badge public">Público</span></div>
          ...
          <section id="ubicacion" className="profile-editor-card reveal-card">
            <div className="profile-card-heading"><div><span className="profile-section-number">02</span><div><h2>Ubicación</h2><p>Seleccioná tu punto. Se guarda siempre de forma privada y solo se publica si vos lo autorizás.</p></div></div><span className={`profile-privacy-badge ${profile.locationPublic ? "public" : "private"}`}>{profile.locationPublic ? "Público" : "Privado"}</span></div>
            ...
            <label className="location-public-consent">
              <input type="checkbox" checked={profile.locationPublic} onChange={(event) => patchProfile("locationPublic", event.target.checked)} />
              <span><strong>Permitir que esta ubicación sea pública</strong><small>Al activarlo, tu punto aparecerá en tu perfil público y en el mapa de Germina. Si está desactivado, solo vos podés verlo.</small></span>
            </label>
          </section>

          <section id="servicios" className="profile-editor-card reveal-card">
            <div className="profile-card-heading"><div><span className="profile-section-number">03</span><div><h2>Servicios y contenido</h2><p>Mostrá visualmente lo que hacés o lo que ofrece tu emprendimiento.</p></div></div><span className="profile-privacy-badge public">Público</span></div>
            ...
            <div className="portfolio-upload-panel">
              <div className="portfolio-upload-fields"><label><span>Título del contenido</span><input value={portfolioTitle} onChange={(event) => setPortfolioTitle(event.target.value)} placeholder="Ej. Identidad visual Café Norte" /></label><label><span>Descripción breve</span><input value={portfolioDescription} onChange={(event) => setPortfolioDescription(event.target.value)} placeholder="Qué hiciste, producto o servicio mostrado" /></label></div>
              ...
              <button type="button" className="portfolio-drop-button" disabled={uploadingPortfolio} onClick={() => portfolioInputRef.current?.click()}>{uploadingPortfolio ? <LoaderCircle className="spin" size={25} /> : <ImagePlus size={25} />}<strong>{uploadingPortfolio ? "Subiendo contenido..." : "Agregar imágenes de tus servicios"}</strong><span>JPG, PNG o WebP · hasta 10 MB por imagen</span></button>
            </div>
          ...
          <section id="redes" className="profile-editor-card reveal-card">
            <div className="profile-card-heading"><div><span className="profile-section-number">04</span><div><h2>Redes y contacto</h2><p>Conectá tu presencia digital con tu perfil Germina.</p></div></div><span className="profile-privacy-badge public">Público</span></div>
          ...
          <section id="legal" className="profile-editor-card legal-private-card reveal-card">
            <div className="legal-security-banner"><span><LockKeyhole size={20} /></span><div><strong>Área privada y confidencial</strong><p>Estos documentos no forman parte de tu perfil público. Solo vos y el equipo administrativo autorizado podrán acceder a ellos.</p></div></div>
            <div className="profile-card-heading"><div><span className="profile-section-number">05</span><div><h2>Documentación legal</h2><p>Guardá documentación para procesos futuros de validación.</p></div></div><span className="profile-privacy-badge private"><ShieldCheck size={13} /> Privado</span></div>
            <div className="legal-upload-row"><label><span>Tipo de documento</span><select value={legalType} onChange={(event) => setLegalType(event.target.value)}><option>Identificación</option><option>RUC / Registro fiscal</option><option>Constancia de negocio</option><option>Licencia / permiso</option><option>Otro documento</option></select></label><input ref={legalInputRef} hidden type="file" accept="application/pdf,image/jpeg,image/png" onChange={(event) => event.target.files?.[0] && uploadLegal(event.target.files[0])} /><button type="button" className="legal-upload-button" disabled={uploadingLegal} onClick={() => legalInputRef.current?.click()}><UploadCloud size={18} /> {uploadingLegal ? "Subiendo..." : "Subir documento"}</button></div>
            <p className="legal-helper">PDF, JPG o PNG · máximo 15 MB.</p>
            <div className="legal-document-list">{legalDocuments.length ? legalDocuments.map((item) => <article key={item.id}><span className="legal-file-icon"><FileText size={19} /></span><div><strong>{item.documentType}</strong><p>{item.fileName} · {(item.size / 1024 / 1024).toFixed(2)} MB</p></div><button type="button" onClick={() => openLegalDocument(item)}>Ver</button><button type="button" className="danger" onClick={() => removeLegalDocument(item)} aria-label="Eliminar documento"><Trash2 size={15} /></button></article>) : <div className="legal-empty"><FileCheck2 size={22} /><span>No has subido documentos legales.</span></div>}</div>
          </section>

          <div className="profile-save-bar"><div><strong>Guardá tus cambios</strong><span>La ubicación se conserva aunque decidas mantenerla privada.</span></div><button type="button" className="btn btn-primary btn-lg" disabled={saving} onClick={saveProfile}>{saving ? <><LoaderCircle className="spin" size={18} /> Guardando...</> : <><Save size={18} /> Guardar perfil</>}</button></div>
        </div>
      </div>
    </main>
  );
